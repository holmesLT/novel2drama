...
- **一键成片**：`python3 pipeline.py 小说.txt` 从小说直达成片，也可以分步执行
- **零依赖**：只用 Python 标准库 + ffmpeg（macOS：`brew install ffmpeg-full`）
- **人物一致性**：锁定外貌（`appearance`）+ 定妆照（CogView 生成）+ 图生视频首帧锁定
- **画面连贯**：场景环境锁定 + 尾帧接续（下一镜头从上一镜头的末帧继续）
- **成片完整度**：台词字幕烧录、程序化环境音/BGM 混音、片头片尾卡、碎剪节奏
- **横竖屏**：`--aspect 16:9 / 9:16`，一条参数适配短视频平台
- **长文自动处理**：按段落分块改编、跨块角色名自动归并（LLM 剧统筹）
- **任意模型可换**：任何 OpenAI 兼容接口；默认配置全程免费（glm-4-flash + cogview-3-flash + cogvideox-flash）
- **断点续渲**：片段全部缓存，单镜头可 `--shots` 定点重渲，网络超时自动重试
...
| --- | --- | --- |
| 第一步 | 小说 → 结构化剧本 | ✅ |
| 第二步 | 剧本 → 分镜脚本（镜头、时长、提示词、环境/画风锁定） | ✅ |
| 第三步 | 分镜 → 成片（定妆照、图生视频、TTS、字幕、BGM、片头尾卡） | ✅ |
| 进阶 | 多集批量 / 竖屏优化 / 角色参考图网格 / 更强视频模型适配 | 📋 欢迎共建 |
